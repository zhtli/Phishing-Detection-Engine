...
class active_rules:
    ...
    def goog_safe_browsing(self, domain_features):
        ...
        try:
            ...
            #phishing_url_list = self.get_urls(self.google_sb_query(list, sep_list.index(list), len(domain_features)))  # active mode

            phishing_url_list = self.get_urls(json.loads(open("constant/gb_phish.json", "r").read()))  # from file
    ...
    def google_sb_query(self, url_list, count, overall_count):
        ...
        for list in sep_list:
            ...
            if 'matches' in r.keys():
                ...
                self.logger.debug("Google-SB query error - Total processed samples: " + str(overall_count) + "\nProcessed chunk (500): " + str(count))
