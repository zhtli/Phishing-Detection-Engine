...
1. **Cron-driven collection** (via [`../cli/`](../cli/README.md) → `MongoStore`): pull URL
   lists and enrich stored URLs with raw domain records / page content. These paths are
...
| `domain_record.py` | `collect_domain_record` (async) / `collect_domain_record_sync` — assemble DNS + IP + RDAP into the one canonical raw domain-record dict that both the domain stage and the enrichment collector use. |
