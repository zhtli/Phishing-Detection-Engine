...
class nlp_class:
    ...
    def __init__(self):
        ...
        self.path_data = os.path.join(_here, "input") + os.sep
        self.name_keywords = "keywords.txt"
        self.name_brand_file = "allbrands.txt"
        self.name_random_model = os.path.join(_here, "gib_model.pki")

        self.model_mat = None
        self.threshold = None
        if os.path.exists(self.name_random_model):
        ...
        try:
            for word in words:
                ...
                if word in self.keywords:
                    ...
                    if self.model_mat is None or self.threshold is None:
                        len_lt_7.append(word)
                    elif (gib_detect_train.avg_transition_prob(word, self.model_mat) > self.threshold) == False:
                        dga_in_words.append(word)
                        # TODO: clean words that are similar to keywords
                    elif len(word) < 7:
                        len_lt_7.append(word)
                    else:
            ...
            self.logger.debug(str(words) + " caused an error during processing")
    ...
    def fraud_analysis(self, grouped_words, splitted_words):
        ...
        try:
            ...
            self.logger.debug(str(word_list) + " caused an error during processing")
            self.logger.error("Error : {0}".format(format_exc()))
        return result
