...
class domain_parser(object):
    ...
    def parse(self, domain_list, class_info, count, url_metadata=None):
        ...
        for line in domain_list:
            ...
            tmp = line[line.find(extracted_domain.suffix):len(line)]  # parse path starting from first '/' after TLD
            pth = tmp.partition("/")
            ...
            record = None
            if url_metadata:
                record = url_metadata.get(raw_line)
                if record is None and raw_line != line:
                    record = url_metadata.get(line)

            if record:
                domain['dataset_meta'] = record
    ...
    def parse_nonlabeled_samples(self, domain_list, count=0):
        ...
        for line in domain_list:
            ...
            domain['path'] = pth[1] + pth[2]
            # domain['path'].append(pth[1] + pth[2]) # path can be stored as list and used for statistics
